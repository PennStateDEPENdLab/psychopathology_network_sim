---
title: "Network analyses of psychopathology in cross-section: conceptual and statistical challenges from a latent variable perspective"
...
p_load(knitr, dplyr, ggplot2, tidyr, cowplot, simsem, bootnet, 
       qgraph, abind, foreach, doParallel, pander)
...
For the first simulation, we specify a 2-factor model (9 per factor) with fixed 0.6 loadings. Items y1-y9 loading onto f1, and y10-y18 load onto f2. We then randomly vary the magnitude of a cross-loading of y2 onto f2. 200 replications of this structure with *N*=400 are simulated.
...
#3d plot of sorts
ggplot(bb_rand, aes(x=f1, y=f2, color=value)) + geom_jitter(size=3, position = position_jitter(width=0.03, height=0.03)) + scale_color_gradient() + 
  ggtitle("Association of f1 (x) and f2 (y) loadings with strength centrality (color)")
```
